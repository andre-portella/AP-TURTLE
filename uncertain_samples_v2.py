...
def main():
    ...
    parser.add_argument("--k", type=int, default=10, help="Porcentagem de amostras incertas")
    ...
    num_samples = (int) ((K * 0.01) *  len(Zs_train[0]))

    print(f'percent = {num_samples}')
    ...
    with torch.no_grad():
        ...
        # ===========================================================
        # 3. Combinação das probabilidades (considerando 1 ou mais classificadores)
        # ===========================================================
        if len(label_per_space_train) == 1:
            # Apenas um classificador: usar diretamente suas probabilidades
            combined = label_per_space_train[0]
        else:
            # Combinação via média logarítmica (minimizando soma de divergências KL)
            # log_probs = torch.stack([torch.log(p + 1e-12) for p in label_per_space_train])
            # combined = torch.exp(log_probs.mean(dim=0))
            # combined = combined / combined.sum(dim=1, keepdim=True)
            combined = torch.stack(label_per_space_train).mean(dim=0)
            combined = combined / combined.sum(dim=1, keepdim=True)

        # ===========================================================
        # 4. Incerteza (KL × Entropia)
        # ===========================================================
        # Entropia: mede o quanto a distribuição é incerta
        # entropy = -(combined * torch.log(combined)).sum(dim=1)

        # Entropia individual de cada classificador
        entropies = [-(p * torch.log(p + 1e-12)).sum(dim=1) for p in label_per_space_train]

        # Média das entropias individuais
        entropy = torch.stack(entropies).mean(dim=0)

        # Divergência KL média entre cada classificador e a distribuição combinada
        if len(label_per_space_train) > 1:
            kl_divs = []
            for p in label_per_space_train:
                kl = (p * (torch.log(p + 1e-12) - torch.log(combined + 1e-12))).sum(dim=1)
                kl_divs.append(kl)
            kl_div = torch.stack(kl_divs).mean(dim=0)

        # Produto entre KL e entropia define a incerteza final
        # Se houver apenas 1 classificador, usa só a entropia
        if len(label_per_space_train) == 1:
            uncertainty = entropy
        else:
            uncertainty = kl_div * entropy
        ...
        torch.save(labels_train_aligned_tensor, os.path.join(save_dir, f"labels_train_aligned_{dataset}_{K}.pt"))
        torch.save(labels_train_original_tensor, os.path.join(save_dir, f"labels_train_original_{dataset}_{K}.pt"))
        torch.save(indices_mais_incertos, os.path.join(save_dir, f"indices_mais_incertos_{dataset}_{K}.pt"))
        indices_mais_incertos

        print(f"\nArquivos salvos:")
        print(f" - Alinhado com ground truth: {os.path.join(save_dir, f'labels_train_aligned_{dataset}_{K}.pt')}")
        print(f" - Original (não alinhado):  {os.path.join(save_dir, f'labels_train_original_{dataset}_{K}.pt')}")
        # # ===========================================================
        ...
        # Salvar índices
        torch.save(indices_mais_incertos, os.path.join(save_dir, f"indices_incertos_{dataset}_{K}.pt"))
        print(f" - Indicoes mais incertos:  {os.path.join(save_dir, f'indices_incertos_{dataset}_{K}.pt')}")
